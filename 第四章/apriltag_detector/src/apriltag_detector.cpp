...
apriltag_detector::DetectionTag tag_msgs;
...
{
	ParamGet();
	tag_pub = node_.advertise<apriltag_detector::DetectionTag>("tag_detector",10);
...
void TagDetector::TagPublisher(string* Available_Tag_,string baseFrame,int tag_num,const int worker_num)
{
	...
	for(int i=0; i<worker_num ; i++)
		worker[i] = boost::thread(&TagDetector::WorkerThread,baseFrame);
...
void TagDetector::WorkerThread(string baseFrame)
{
	ros::Rate rate(10);
	while(ros::ok())
	{
