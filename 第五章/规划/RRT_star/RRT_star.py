...
class RRTStar(RRT):# 这里作为子类继承了RRT类
    ...
    def planning(self, animation=True):
        ...
        for i in range(self.max_iter):# 最大的迭代次数
            print("Iter:", i, ", number of nodes:", len(self.node_list))# 迭代次数以及对应的节点列表打印
            rnd = self.get_random_node()# 随机获得新的node信息
            nearest_ind = self.get_nearest_node_index(self.node_list, rnd)# 获取两个节点最近的节点
            ...
            if self.check_collision(new_node, self.obstacle_list):# 检查是否发生碰撞
                ...
                if node_with_updated_parent:# 如果找到了最近的节点
                    self.rewire(node_with_updated_parent, near_inds)# 更新对应的node节点以及对应的父节点的cost
                    self.node_list.append(node_with_updated_parent)# 将该节点加入到节点列表中
                else:
    ...
    def choose_parent(self, new_node, near_inds):
        ...
        for i in near_inds:
            ...
            if t_node and self.check_collision(t_node, self.obstacle_list):# 检查是否发生碰撞
                costs.append(self.calc_new_cost(near_node, new_node))# 计算出新的代价信息
            else:
        ...
        new_node = self.steer(self.node_list[min_ind], new_node)# 通过该函数完成了下一个节点的选取
    ...
    def find_near_nodes(self, new_node):
        ...
        r = self.connect_circle_dist * math.sqrt((math.log(nnode) / nnode))# 计算出该节点的有效半径
        ...
        near_inds = [dist_list.index(i) for i in dist_list if i <= r**2]# 并计算出在r范围内的节点索引
        return near_inds
    ...
    def rewire(self, new_node, near_inds):
        ...
        for i in near_inds:
            ...
            if not edge_node:
                continue
            edge_node.cost = self.calc_new_cost(new_node, near_node)#计算新的代价函数

            no_collision = self.check_collision(edge_node, self.obstacle_list)#计算是否发生碰撞
